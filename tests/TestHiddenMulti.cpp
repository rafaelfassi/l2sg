          ...
          const std::string &expectedBoard)
{
    ...
    bool changed = solver::techniques::hiddenMulti(grid, 2, maxMultiplicity);
...
int main(int, char **)
{
    // Hidden pair row for values 6, 7 at:
    // (1,5) (1,8)
    if (!test(2, "6.5...14.914.5..3..37........3941..2..23.5914149.28..3..1...39..78.9.4.1.96...7.8",
              R"(
                .------------.-----------------.----------------.
    ...
    // Hidden pair row/block for values 4, 7 at:
    // (0,0) (0,1) .....
    // ..... ..... .....
    // ..... ..... .....
    if (!test(2, "....6........42736..673..4..94....68....964.76.7.5.9231......85.6..8.271..5.1..94",
              ...
              R"(
                .--------------------.-----------------.-------------.
                | 47    47     12389 | 1589   6   1589 | 158  15  29 |
                | 589   158    189   | 1589   4   2    | 7    3   6  |
                | 2589  1258   6     | 7      3   1589 | 158  4   29 |
                :--------------------+-----------------+-------------:
                | 235   9      4     | 123    27  137  | 15   6   8  |
    ...
    // Hidden pair column/block for values 3, 4 at:
    // ..... ..... .....
    // ..... (1,4) .....
    // ..... (2,4) .....
    if (!test(2, "9.8.6...41.......875.8.912....3...5..7.9.4.8..1...2....81..3.493...9....6...2...5",
              R"(
                .----------------.--------------------.-------------------.
              ...
              R"(
                .----------------.-------------------.-------------------.
                | 9   23    8    | 1257   6    157   | 357    37    4    |
                | 1   2346  246  | 257    34   57    | 35679  3679  8    |
                | 7   5     46   | 8      34   9     | 1      2     36   |
                :----------------+-------------------+-------------------:
                | 48  269   269  | 3      178  67    | 24679  5     1267 |
                | 25  7     2356 | 9      15   4     | 236    8     1236 |
    ...
    // Hidden pair block for values 6, 9 at:
    // ..... ..... .....
    // ..... (4,7) .....
    // ..... ..... (5,8)
    if (!test(2, ".1.........3...6..7.6284.3..68.29..52..4.5..84..8...2..8.693..7..7...9.........5.",
              R"(
                .------------------.-----------------.--------------------.
              ...
              R"(
                ...
                :------------------+-----------------+--------------------:
                | 13    6     8    | 137   2     9   | 1347   147   5     |
                | 2     379   19   | 4     1367  5   | 137    69    8     |
                | 4     3579  159  | 8     1367  167 | 137    2     69    |
                :------------------+-----------------+--------------------:
                | 15    8     1245 | 6     9     3   | 124    14    7     |
                | 136   234   7    | 15    145   28  | 9      1468  12346 |
                | 1369  2349  1249 | 17    147   28  | 12348  5     12346 |
    ...
    // Hidden triple row for values 1, 2, 7 at:
    // (6,1) (6,2) (6,4)
    if (!test(3, "965147823148.....77..685194...2.93.838.75.2.9291834.7......89..8.....7.26.957.481",
              R"(
                .---------------.--------------.-------------.
                | 9   6    5    | 1    4    7  | 8   2    3  |
    ...
    if (!test(3, "5..62..37..489........5....93........2....6.57.......3.....9.........7..68.57...2",
              ...
              R"(
                ...
                | 1238  1679  1236789 | 1347   5      1347  | 12489  124689  14689 |
                :---------------------+---------------------+----------------------:
                | 9     3     1568    | 1247   1468   256   | 1248   12478   148   |
                | 148   2     18      | 13479  1348   13478 | 6      14789   5     |
                | 7     1456  1568    | 1249   1468   256   | 12489  12489   3     |
                :---------------------+---------------------+----------------------:
                | 1234  1457  12357   | 1234   13468  9     | 13458  14568   1468  |
                | 1234  1459  12359   | 1234   13468  26    | 7      145689  14689 |
                | 6     8     139     | 5      7      134   | 1349   149     2     |
                '---------------------'---------------------'----------------------'
              )"))
        return failed();

    // Hidden triple column for values 5, 7, 9 at:
    // (0,3)
    // (5,3)
    // (8,3)
    if (!test(3, "478.2.61.9.14.827.5.2.7...96.483...27.9.4.3..823.15.64197.8.5.6285.....7346....2.",
              R"(
                .----------.---------------.--------------.
    ...
    if (!test(3, "28....473534827196.71.34.8.3..5...4....34..6.46.79.31..9.2.3654..3..9821....8.937",
              ...
              R"(
                ...
                | 178   9    78    | 2    17   3    | 6    5  4    |
                | 67    45   3     | 46   567  9    | 8    2  1    |
                | 16    245  25    | 146  8    156  | 9    3  7    |
                '------------------'----------------'--------------'
              )"))
        return failed();

    // Hidden quadruple row for values 1,5,6,7 at:
    // (4,0) (4,2) (4,6) (4,8)
    if (!test(4, "6.1.7..3..3.5.176..57936.1.41..658.............871..43.9.6.835.3.6157.8...5.....6",
              R"(
                .----------------.------------------.---------------.
              ...
              R"(
                ...
                :---------------+------------------+---------------:
                | 4    1    239 | 23    6     5    | 8    279  27  |
                | 57   67   239 | 2348  2489  2349 | 156  29   15  |
                | 259  26   8   | 7     1     29   | 56   4    3   |
                :---------------+------------------+---------------:
                | 17   9    24  | 6     24    8    | 3    5    17  |
                | 3    24   6   | 1     5     7    | 249  8    249 |
    ...
    if (!test(4, ".3.....1...8.9....4..6.8......57694....98352....124...276..519....7.9....95...47.",
              ...
              R"(
                .------------------.---------------.------------------.
                | 5679  3     279  | 24   45   27  | 2678  1     2459 |
                | 567   1256  8    | 234  9    127 | 2367  356   245  |
                | 4     125   1279 | 6    135  8   | 237   35    259  |
                :------------------+---------------+------------------:
                | 138   128   123  | 5    7    6   | 9     4     138  |
                | 67    146   147  | 9    8    3   | 5     2     167  |
                | 5679  568   379  | 1    2    4   | 3678  368   3678 |
                :------------------+---------------+------------------:
                | 2     7     6    | 348  34   5   | 1     9     38   |
                | 138   148   134  | 7    136  9   | 2368  3568  25   |
                | 138   9     5    | 238  136  12  | 4     7     368  |
                '------------------'---------------'------------------'
              )"))
        return failed();
    ...
    if (!test(4, "816573294392......4572.9..6941...5687854961236238...4.279.....1138....7.564....82",
              ...
              R"(
                ...
                | 6  2  3 | 8     15    15  | 79  4   79 |
                :---------+-----------------+------------:
                | 2  7  9 | 36    458   458 | 46  35  1  |
                | 1  3  8 | 69    245   245 | 46  7   59 |
                | 5  6  4 | 1379  13    17  | 39  8   2  |
                '---------'-----------------'------------'
              )"))
        return failed();
